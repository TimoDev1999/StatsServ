...
func GetOrderBook(c *gin.Context, db *sql.DB) {
	exchangeName := c.Param("exchange_name")
	pair := c.Param("pair")

	var orderBook models.OrderBook
	row := db.QueryRow(`SELECT id, exchange, pair, asks, bids 
                        FROM order_book 
...
func SaveOrderBook(c *gin.Context, db *sql.DB) {
	var orderBook models.OrderBook
	...
	_, err := db.Exec(`INSERT INTO order_book (exchange, pair, asks, bids) VALUES ($1, $2, $3, $4)`,
...
func GetOrderHistory(c *gin.Context, db *sql.DB) {
	clientName := c.Query("client_name")
	exchangeName := c.Query("exchange_name")
	rows, err := db.Query(`SELECT client_name, exchange_name, label, pair, side, type, base_qty, price, algorithm_name_placed, lowest_sell_prc, highest_buy_prc, commission_quote_qty, time_placed
                            FROM order_history
...
func SaveOrder(c *gin.Context, db *sql.DB) {
	var order models.HistoryOrder
	if err := c.ShouldBindJSON(&order); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	_, err := db.Exec(`INSERT INTO order_history (client_name, exchange_name, label, pair, side, type, base_qty, price, 
